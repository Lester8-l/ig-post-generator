...
**關鍵字要涵蓋「更長的衍生詞」，否則短詞會把長詞切壞。**
機制是「長詞優先」匹配，但如果清單裡只有短詞，它就會只配到那幾個字：清單有
`方法` 而文中是「**方法**論」→ 藥丸會變成「方法（金）＋論（綠）」，看起來像打錯字。
做法是把 `方法論` 一併列進去（順序無所謂，程式會自己按長度排序）。

選關鍵字時的檢查動作：對每個候選詞，在文中搜一次，看它有沒有更長的複合詞
（`方法` → `方法論`、`成功` → `成功學`、`系統` → `系統化`…），有就一起列。
